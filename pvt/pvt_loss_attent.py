...
parser = argparse.ArgumentParser(description='Script for training and evaluating PVT model with the proposed loss')
...
#Calculating the size of three sets
...
class CustomLoss_np(nn.Module):
    ...
    def forward(self, images, outputs, labels, predictions):
        ...
        for i in range(images.size(0)):  
            ...
            # Calculate the attention loss of grad-cam
            image_tensor = images[i].unsqueeze(0)
            predict = predictions[i].item()
            targets = [ClassifierOutputTarget(predict)] # Obtain heatmap from predicted class
            grayscale_cam = cam(input_tensor=image_tensor, targets=targets)
            grayscale_cam = grayscale_cam[0, :]
...
#             threshold=args.thresh
            ...
            red_area_ratio_target = red_pixels_target / total_pixels_target
            loss_attent = (1 - red_area_ratio_target) * labels[i].item() # attention loss = (1-ratio) * gt label
...
#           threshold=args.thresh
          if labels[i].item()==0:
            ...
            bottom_right = grayscale_cam[height // 2:, width // 2:]
            bottom_right_tensor = torch.from_numpy(bottom_right)  # Converting numpy arrays to PyTorch tensor
            red_pixels_target = torch.sum(bottom_right_tensor > threshold) # 0.3 is a threshold
            total_pixels_target = bottom_right_tensor.numel()
            red_area_ratio_target = red_pixels_target / total_pixels_target
