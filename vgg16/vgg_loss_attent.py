...
parser = argparse.ArgumentParser(description='Script for training and evaluating VGG16 model with the proposed loss')
...
class CustomLoss_ne(nn.Module):
    ...
    def forward(self, images, outputs, labels, predictions):
        ...
        for i in range(images.size(0)):  # loop the batch dimension
...
#             threshold = 0.9
            ...
            loss_ce = F.cross_entropy(outputs[i], labels[i])
#             lamda=0.75
            #loss_combined = loss_ce + loss_attent
            loss_combined = lamda*loss_ce + (1-lamda)*loss_attent
            losses[i] = loss_combined
...
# Custom loss for multiple classes: n, e, p
